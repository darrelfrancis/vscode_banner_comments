  ...
  vscode.window
    ...
    .then((_selectedPickerItem: vscode.QuickPickItem) => {
      ...
      editor.edit((builder: vscode.TextEditorEdit) => {
        editor.selections.forEach(_selection =>
          replaceSelectionWithBanner(
            editor.document,
            builder,
            _selection,
            figletConfig,
  ...
  getFavoriteFontFromUser((err, font) => {
    if (err) {
      vscode.window.showErrorMessage(
        "Banner-comments: An error occurred while getting a favorite font from user! See the logs for more information."
      );
...
function getFavoriteFontFromUser(callback) {
  ...
  vscode.window
    .showQuickPick(quickPickFavorites)
    .then((_selectedPickerItem: vscode.QuickPickItem) => {
...
function getLanguageConfig(languageId: string): any {
  ...
  /**
   * unfortunately, some of vscode's language config contains
...
export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(
     ...
     */
    vscode.commands.registerCommand("extension.bannerCommentApplyFromList", _ =>
      applyFromList()
    ),
    vscode.commands.registerCommand("extension.bannerCommentApplyH1", _ =>
      applyFromHeader("h1")
    ),
    vscode.commands.registerCommand("extension.bannerCommentApplyH2", _ =>
      applyFromHeader("h2")
    ),
    vscode.commands.registerCommand("extension.bannerCommentApplyH3", _ =>
      applyFromHeader("h3")
    ),
    vscode.commands.registerCommand("extension.bannerCommentApplyFavorite", _ =>
      applyFromFavorite()
    ),
    /**
     * Banner-comment command to set the font and save it to the workspace configuration.
     */
    vscode.commands.registerCommand("extension.bannerCommentSetH1Font", _ =>
      setHeaderFont("h1")
    ),
    vscode.commands.registerCommand("extension.bannerCommentSetH2Font", _ =>
      setHeaderFont("h2")
    ),
    vscode.commands.registerCommand("extension.bannerCommentSetH3Font", _ =>
      setHeaderFont("h3")
    ),
    /**
     * Banner-comment command to add a font to the list of favorites which saves into workspace configuration.
     */
    vscode.commands.registerCommand("extension.bannerCommentAddToFavorite", _ =>
      addAFontToFavorites()
    ),
    vscode.commands.registerCommand(
      "extension.bannerCommentRemoveFromFavorite",
